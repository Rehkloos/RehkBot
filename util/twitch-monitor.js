...
module.exports = async (client) => {
    ...
    L.log('[Twitch] Monitoring stream');
    new CronJob('0 * * * * *', async () => {
        ...
        if (!stream) {
            ...
            await channel.messages
                ...
                .then((msg) => msg.edit(`${user.display_name}` + " is now offline!", {
                    embed
                }))
                .catch(() => channel.send(`${user.display_name}` + " is now offline", {
                    embed
                }));
        ...
        if (!streamData) {
            ...
            const streamMessage = await channel.send(`${user.display_name}` + " is now online!", {
                embed
            });
        ...
        let streamMessage = streamData.message_id ?
            await channel.messages
            .fetch(streamData.message_id)
            .then((msg) => msg.edit(`${user.display_name}` + " is now online!", {
                embed
            }))
            .catch(() => channel.send(`${user.display_name}` + " is now online!", {
                embed
            })) :
            await channel.send(`${user.display_name}` + " is now online!", {
                embed
            });
        //? Update DB
